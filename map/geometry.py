...
class Grid(object):
    ...
    def screen_to_grid(self, offset, point, size):
        return (
            (point[0] - offset[0]) // size,
            (point[1] - offset[1]) // size,
        )
    ...
    def neighbors(self, pos, cell_filter = None):
        (x, y) = pos
        results = [(x+1, y), (x, y-1), (x-1, y), (x, y+1)]
        if (x + y) % 2 == 0: results.reverse() # aesthetics
        results = filter(self.in_bounds, results)
        if not cell_filter is None:
            results = filter(cell_filter, results)
        return results

    def neighbors_diag(self, pos, cell_filter = None):
        (x, y) = pos
        results = [
